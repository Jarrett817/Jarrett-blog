...
<span class="line"><span style="color:#93A1A1;font-weight:bold;">let</span><span style="color:#268BD2;"> evenNumber</span><span style="color:#859900;">:</span><span style="color:#CB4B16;"> EvenNumbers</span><span style="color:#859900;"> =</span><span style="color:#D33682;"> 2</span><span style="color:#839496;">;</span></span></code></pre></div><p>此例中，<code>EvenNumbers</code> 是从 <code>AllNumbers</code> 联合类型中提取出偶数 2 和 4 后的类型。</p><h2 id="业务应用实践" tabindex="-1">业务应用实践 <a class="header-anchor" href="#业务应用实践" aria-label="Permalink to &quot;业务应用实践&quot;">​</a></h2>`,35),P=p('<p>以上是静态类型校验，那么如何过渡到动态类型校验？</p><h2 id="运行时数据校验" tabindex="-1">运行时数据校验 <a class="header-anchor" href="#运行时数据校验" aria-label="Permalink to &quot;运行时数据校验&quot;">​</a></h2><div class="warning custom-block"><p class="custom-block-title">解决什么问题</p><ul><li>后端服务互相调用，数据类型不确定</li><li>可能出现的属性空值</li><li>可能由类型导致的问题，比如 falsy 判断、字符串和数字相加</li></ul></div><h3 id="碰到的挑战" tabindex="-1">碰到的挑战 <a class="header-anchor" href="#碰到的挑战" aria-label="Permalink to &quot;碰到的挑战&quot;">​</a></h3><p><img src="'+B+'" alt="使用组件" loading="lazy"></p><p>经过调研，选用了 zod 库作为方案实现的核心库</p><p>该库较为成熟，社区活跃，原生支持 TS，且能够完全对标 TS 语法，一份 schema 同时生成校验器和 TS 静态类型，使用简单、直观、轻便</p>',7),E=p(`<div class="vp-code-group"><div class="tabs"><input type="radio" name="group-0-ftP" id="tab-Y7uwszt" checked><label data-title="/base-service/validator.ts" for="tab-Y7uwszt">/base-service/validator.ts</label><input type="radio" name="group-0-ftP" id="tab-85h228c"><label data-title="/detail-service/api.ts" for="tab-85h228c">/detail-service/api.ts</label><input type="radio" name="group-0-ftP" id="tab-OfC7aKt"><label data-title="/detail-service/validator.ts" for="tab-OfC7aKt">/detail-service/validator.ts</label><input type="radio" name="group-0-ftP" id="tab-aBq2RFd"><label data-title="/detail-service/types.ts" for="tab-aBq2RFd">/detail-service/types.ts</label></div><div class="blocks"><div class="language-ts active"><button title="Copy Code" class="copy"></button><span class="lang">ts</span><pre class="shiki solarized-dark vp-code" tabindex="0"><code><span class="line"><span style="color:#93A1A1;font-weight:bold;">const</span><span style="color:#268BD2;"> safeString</span><span style="color:#859900;"> =</span><span style="color:#839496;"> () </span><span style="color:#93A1A1;font-weight:bold;">=&gt;</span></span>
...
`;return(q,N)=>{const n=u("CodeEditor");return A(),g("div",null,[D,a(n,{value:c}),b,h,a(n,{value:o}),m,f,a(n,{value:e}),C,S,x,a(n,{value:t}),T,_,v,a(n,{value:r}),w,a(i),a(n,{value:y}),P,a(d),E])}}});export{z as __pageData,L as default};
